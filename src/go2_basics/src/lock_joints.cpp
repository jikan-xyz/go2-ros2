...
{
public:
    RobotNode() : Node("go2_lock"), timer_count_(0)
    {
...
private:
    void send_request()
    {
        // Créer la requête StandUp
        unitree_api::msg::Request req;
        sport_req_->StandUp(req);

        // Publier la requête pour lever le robot
        req_puber_->publish(req);
        RCLCPP_INFO(this->get_logger(), "Commande 'StandUp' envoyée au robot (oui standup pour lock).");
